function ScoreBoardGameControl (){
	...
	this.incrementScore =  function (){
		corrects++;
		score+= POINT_GAME;
		if (corrects ==  TOTAL_CORRECT){ //se todos forem achados, avisar usuario
			alert("Parabéns! Você ganhou! Seus pontos foram " + score);
		}
	}

	this.decrementScore =  function (){ // se não quiser decremento do score, tirar essa função
		score-= POINT_GAME;
		if (score < 0) { // para não negativar o score
			score = 0;
		}
	}
}
...
function Card(picture){
	var FOLDER_IMAGES = 'imagens/' // pasta onde todas imagens devem estar
	var IMAGE_QUESTION  = "questionMark.png" // imagem do "?"
	this.picture = picture;
	this.visible = false;
	this.block = false;

	this.equals =  function (cardGame){
		if (this.picture.valueOf() == cardGame.picture.valueOf()){ // se a carta for igual retorna true
			return true;
		}
